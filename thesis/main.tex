...
\begin{figure}[h]
\centering
\includegraphics[scale=0.3]{images/bunny_bvh1.png}
...
\item
Unterscheidet sich die Qualität der BVHs zwischen direktem k-way splitting und collapse einer binären BVH abhängig von der gewählten statischen Szene?
\item
Wie groß ist der Trade-off zwischen Construction- und Traversal-Time mit Bezug auf das gewählte Splitting-Verfahren jeweils über $k \in \{2, 4, 8, 16\}$ in statischen Szenen?
\end{itemize}
...
Innerhalb der Szenen wird eine einfache Kamerafahrt in 36 Snapshots unterteilt. Bei der Kamerafahrt handelt es sich um eine einfachen Pfad um den Ursprung der Szene, bei dem dieser stehts im Mittelpunkt steht. In den meisten Szenen rotiert damit die Kamera um ein Objekt. 
Dabei werden als Hauptmetriken die Construction-Time und die Traversal-Time erfasst und verglichen. Zudem wird die Anzahl an \emph{Hit Rays}, also die Anzahl an Rays, die tatsächlich mit einem Polygon kollidieren, erfasst. Dies geschieht um die Modelle anhand ihrer Polygonanzahl zu vergleichen, obwohl diese unterschiedlich skaliert sind. Die Tests erfolgen lediglich auf der CPU.
...
- k = 4 als Sieger des statischen und 

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{results/statistical_results_table.png}
\caption{Detailierte Ergebisse}
\end{figure}
