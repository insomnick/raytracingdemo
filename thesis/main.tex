...
Diese Arbeit untersucht Top-Down konstruierte BVH in statischen dreidimensionalen Szenen für Verzweigungsgrade $k \in \{2, 4 ,8 ,16\}$. Dies geschieht unter Berücksichtigung von drei Splitting-Verfahren (Median, Surface Area Heuristic und Binned Surface Area Heuristic), sowie zwei Methoden zur Erzeugung von Wide BVHs (k-way splitting und binary BVH collaping). Dabei steht die Traversal-Time als primäres und die Construction-Time als sekundäres Qualitätsmerkmal im Fokus. Zudem wird durch eine Kombination der beiden Qualitätsmetriken das Szenario einer dynamischer Szene modelliert.

Für statische Szenen zeigt $k = 4$ gegenüber der referenz $k = 2$ keinen einheitlichen Trend für alle Kombinationen an Parametern. Abhängig von Splitting-Verfahren und Wide BVH Erzeugungsmethode liegen die dazugehörigen Speedups zwischen $0,932$ und $1,234$. Insbesondere der Median-Split in Kombination mit collapsing erzeugt schlechtere Traversal-times gegenüber der binären Referenz, während SAH und BSAH häufiger Verbesserungen aufzeigen. Für größere Verzweigungsgrade verschlechtern sich die Traversal-Times über alle Konfigurationen mit Speedups zwischen $0,529$ und $0,866$ für $k = 8$, sowie nochmal deutlich schlechtere für $k = 16$ mit Speedups zwischen $0,077$ und $0,666$. Dabei ist die Streuung dieser Werte stark abhängig von der gewählten Wide BVH Erzeugungsmethode.
...
Hieraus haben sich drei Problemfragen ergeben, die den Umfang dieser Arbeit definieren:
\begin{itemize} 
\item
...
\item
Unterscheidet sich die Traversal-Time der BVHs zwischen direktem k-way splitting und collapsing einer binären BVH abhängig von der gewählten statischen Szene?
\item
...
In der zugrundeliegenden Arbeit werden lediglich statische dreidimensionale Szenen untersucht. Verfahren die darauf abzielen die BVH dynamisch anzupassen (Refitting etc.) werden nicht verwendet. Trotzdem wird in späteren Kapiteln ein Modell für dynamsiche Szenen auf Grundlage der Datensätze erarbeitet.
...
Die verglichenen Datenstrukturen sind lediglich BVH bzw. Wide BVH. Es werden keine anderen Beschleunigungsstrukturen, wie k-d-Trees betrachtet. Wide BVHs werden im Vergleich zu ihrer binären Referenzimplementation untersucht. 
...
Ziel ist es einzuschätzen, ob die Möglichkeit der parallelisierten Berechnung besser genutzt werden kann, durch z.B. mehr untersuchte Rays oder höhere Auflösungen, statt möglicherweise ineffiziente Erhöhung des Verzweigungsgrads. 

\section{Vorgehensweise}
Zuerst wird in die benötigen Grundlagen und die Terminologie des Raytracings eingeführt. Darunter zählen Rays, AABBs und BVH-Construction, sowie Traversal. Darauf folgend werden die implementierten Algorithmen und der experimentelle Aufbau, wie auch das dazugehörige Datenmodell beschrieben. Nach der Vorstellung der Messergebnisse, werden diese anschließend diskutiert, Limitationen aufgezeigt und die Ergebnisse in den Literaturkontext eingeordnet. Zuletzt wird ein Fazit ausgeführt, welches die zentralen Erkenntnisse zusammenfasst.
...
Für das weitere Vorgehen werden unter Primitives immer Polygone, also Dreiecke, verstanden, da die verwendeten Szenen in diesem Format vorliegen und es auch sonst in den meisten Anwendungsfällen als Standard gesehen wird. Polygone werden durch ihre drei Eckpunkte definiert, die jeweils durch dreidimensionale Vektoren beschrieben werden. Zudem wird für die Vereinfachung von Berechnungen der Mittelpunkt des Polygons bestimmt, \emph{Centroid} genannt. Eine zusammenhängende Struktur aus Polygonen wird als \emph{Mesh} verstanden. 

Wenn sich im Traversal-Schritt die Frage stellt, ob ein Ray ein Primitive schneidet, stellt sich eigentlich die Frage, ob ein bestimmtes Primitive das erste ist, was der untersuchte Ray schneidet. Man spricht hier von einem \emph{closest hit test}~\cite{https://doi.org/10.1111/cgf.142662}.

\section{Axis Aligned Bounding Box}

Um Construction und Traversal von Nodes einer BVH möglichst einfach zu gestalten, müssen einige Punkte gegeben sein: 
\begin{itemize}
    \item Nodes müssen eine möglichst performante Möglichkeit bieten, die mögliche Kollision zwischen einem Ray und ihren Primitives zu überprüfen.
    \item Die räumlichen Grenzen von Nodes sollen möglichst einfach Berechnet werden können.
    \item Nodes sollen möglichst wenig Speicherplatz verbrauchen, bzw. möglichst wenig Zusatzinformationen benötigen.
\end{itemize}
Die am häufigsten verwendete Datenstruktur, die diese Punkte miteinander vereint, ist die Axis Aligned Bounding Box. Man kann sie sich als einen an den Koordinatenachsen ausgerichteten Quader vorstellen, der die darin enthaltenen Primitives möglichst eng einschließt. Die räumlichen Grenzen lassen sich durch zwei Vektoren, die die oberen und unteren Schranken der Box beschreiben leicht definieren. Diese Vektoren lassen sich durch die Grenzen der enthaltenen Primitives effizient ermitteln.
...
Eine BVH ist im Kern eine Baumstruktur, die eine hierarchische Partitionierung der Primitives in der Szene erwirkt. Um einen inneren Node zu definieren, benötigt man eine AABB, sowie Referenzen zu den eingeschlossenen Primitives und Referenzen zu ihren Child-Nodes. Die AABBs von Child-Nodes befinden sich immer innerhalb der Grenzen der AABB ihres Parent-Nodes. Es ist allerdings möglich, dass die AABBs der Child-Nodes sich überschneiden. Die Wurzel ist gleich definiert. Die Leaf-Nodes der Datenstruktur sind konzeptionell jene, deren Menge an Primitives kleiner gleich dem festgelegten Verzweigungsgrad sind. 
...
Die direkte Konsequenzen von Wide BVH sind eine geringere Tiefe und mehr Kollisionschecks mit Child-Nodes pro Node. Vor- und Nachteile bezogen auf Construction und Traversal werden innerhalb dieser Arbeit untersucht.

\subsection{Traversal}
Der Traversal-Schritt ist der Kerngrund für die Verwendung einer BVH. Die Traversal-Time zu minimieren ist ihre Hauptaufgabe. Die Traversal geschieht dabei Top-Down durch die Hierarchie. Falls es eine Kollision zwischen Ray und AABB eines Nodes gibt, sollen auch die Child-Nodes, bzw. bei einem Blatt die Primitives, überprüft werden. Falls nicht, kann der gesamte Teilbaum ausgelassen werden. Die Reihenfolge wird hier durch einen Stack gegeben. In diesem Fall handelt es sich um eine Art der Tiefensuche mit frühem Abbruchkriterium pro Teilbaum.
Die Zeiten für einen AABB-Test und einen Primitive-Test unterscheiden sich, wodurch sich ein weiteres Abbruchkriterium ergeben kann. Dies geschieht, indem man die Zeit für Ray-Primitive-Kollision, die Dauer des Durchsuchens eines Teilbaums und die Menge an Primitives im Teilbaum in eine Kostengleichung einträgt. Es existieren auch Vereinfachungen dieser Gleichung, welche im folgenden Schritt erklärt werden.

\subsection{Kostenmodelle}
Es gibt eine Reihe von sogenannten \emph{Traversal-Cost-Models}~\cite{meister2022performance}. Hierbei gibt es eine \emph{Cost-} bzw. \emph{Loss-Function}, die bei der Konstruktion des Baumes verwendet wird, um den für die Funktion optimalen Split zu finden.
...
Dabei entspricht $c_T$ der durchschnittlichen Zeit für einen Schritt, während der Traversal und $N_{child}$ wird als Iterator über alle Kinder des Nodes $N$ verstanden.
...
Insgesamt werden pro Testdurchlauf $10$ Datenpunkte für die Construction-Time erfasst und dann pro Frame jeweils die Traversal-Time gesammelt. 
Alle Tests wurden $10$ Mal wiederholt, um Messfehler auszuschließen.
Damit kommt man für jede Permutation von Algorithmus, Verzweigungsgrad, Wide BVH Variante und Modell auf $84$ verschiedene Testdurchläufe mit je $46$ Datenpunkten und $10$ Wiederholungen. Das ergibt gesamtheitlich \emph{38.640 Datenpunkte}.
...
Zu jedem Frame wird, wie in Abbildung~\ref{fig:models} dargestellt, ein Bild erzeugt, welches dazu dient die Korrektheit der Traversal zu untersuchen. Damit kann zudem sichergestellt werden, dass die untersuchte BVH eine gültige Form besitzt. Unabhängig vom Testdurchlauf sollen die erzeugten Bilder pro Testszene immer gleich sein.  
...
Die implementierte BVH verwendet nicht direkt Polygone, sondern ist in der Lage Primitives verschiedener Arten zu verarbeiten, solange sie die benötigten Grundeigenschaften besitzen. Dazu gehören ein Mittelpunkt (Center oder auch Centroid), ein Minimal- und Maximalpunkt im Sinne einer Bounding Box, die nur das Primitive selbst einschließt und eine Funktion, um Kollisionen (Intersections) mit Rays zu erkennen und gegebenenfalls den dazugehörigen Punkt im Raum zu bestimmen. 
...
Nachdem eine Kopie der Primitives für die BVH erstellt und sichergegangen wurde, dass überhaupt Primitives in der Szene existieren, wird die \emph{Root-Node} erstellt. Diese wird auf einen Stack gelegt. 
...
\]
Dabei beschreibt $Area$ den Oberflächeninhalt der AABB der Teilmenge. Gewählt wird Split-Index $i$ für den $c(i)$ minimal ist. Das errechnen der kosten geschieht in $O(n)$

Für Wide BVHs wird dieser Vorgang \emph{greedily} mehrfach wiederholt, bis der gewünschte Verzweigungsgrad erreicht ist. Dafür wird das Segment, bzw. die Teilmenge an Primitives für den Algorithmus berücksichtig, welches die aktuell höchsten SAH-Kosten entstehen lässt. Insgesamt entstehen dadurch $k - 1$ Split-Indizes, die $k$ Teilmengen definieren. 
...
\begin{figure}[h]
    ...
    \caption{Traversal-Speedup collapsing gegen k-way splitting \\ (Referenz collapsing)}
...
Damit kann man einen praktischen Unterschied zwischen den Verfahren zur Erzeugung von Wide BVH erkennen und k-way splitting gegenüber collapsing, vor allem für höhere Verzweigungsgrade, für alle untersuchten Szenen als überlegen ansehen.
...
\section{Ausblick}
Fortlaufend bietet es sich an die Stichprobe an Szenen zu erweitern. Mehr und vor allem größere Szenen, sowie komplexere Szenen mit Kameraursprung innerhalb der Szene bieten Forschungsspielraum. Zudem wäre eine Untersuchung, die nicht nur Primärstrahlen überprüft, sondern auch Shadingeffekte, durch z.B. \emph{Ambient Occlusion}, denkbar. Auch wäre eine quantitative Untersuchung der AABB- und Triangle-Tests sinnvoll, um die hier genannten Erklärungen zu untermauern. Auf algorithmischer Ebene wäre eine Untersuchung verschiedener Bin-Größen für BSAH von Interesse, sowie unterschiedliche Abbruchkriterien bei der Konstruktion. 

\newpage
\chapter{Fazit}
Der Fokus dieser Arbeit war die analytische Bewertung der Qualität von Top-Down konstruierten Wide BVHs in statischen 3D Szenen basierend auf serieller Berechnungen. Dabei gab es das Ziel herauszufinden, ob Wide BVH alleinig auf struktureller bzw. algorithmischer Ebene einen Vorteil gegenüber binärer BVHs innehaben, auch wenn der Vorteil der parallelisierten Berechnung wegfällt.
...
Zusammengefasst lässt sich sagen, dass für die serielle Traversal auf der CPU in statsichen Szenen $k = 2$ nicht als robuster Standard abgelöst werden kann. Für Szenen oder Splitting-Algorithmen abhängige Anwendungen kann $k = 4$ sinnvoll sein. Allerdings sind $k \ge 8$ allgemein nicht zu empfehlen, genauso wie die Nutzung von collapsing in höheren Verzweigungsgraden. 
