...
def load_and_process_data(files):
    ...
    if not all_data:
        print("No ray data files found. Creating sample data for demonstration.")
    ...
    total_count = combined_df['count'].sum()
    if total_count == 0:
        print("All hit counts are 0 in the data files. Creating realistic sample data for demonstration.")
        return create_sample_data()
...
def create_stacked_bar_chart(df, include_sah16=True):
    if include_sah16:
        algorithms = ['median-4', 'median-c-4', 'median-8', 'median-c-8', 'median-16', 'median-c-16']
        output_suffix = "_with_median16"
        title_suffix = "Including Median-16"
    else:
        algorithms = ['median-4', 'median-c-4', 'median-8', 'median-c-8']
        output_suffix = "_without_median16"
        title_suffix = "Median-4, Median-8 with Centroid Variants"
    ...
    ax.set_xlabel('BVH Construction Algorithm (Median with different bucket counts)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Number of Intersection Tests per Frame', fontsize=12, fontweight='bold')
    ax.set_title(f'Ray-BVH Intersection Tests Analysis (Stanford Bunny)',
    ...
    output_path = f'hit_ray_analysis{output_suffix}.png'
...
def print_detailed_summary(df, include_sah16=True):
    if include_sah16:
        algorithms = ['median-4', 'median-c-4', 'median-8', 'median-c-8', 'median-16', 'median-c-16']
        title_suffix = "WITH Median-16"
    else:
        algorithms = ['median-4', 'median-c-4', 'median-8', 'median-c-8']
        title_suffix = "WITHOUT Median-16"
    ...
    print("\n" + "="*90)
    print(f"DETAILED RAY-BVH INTERSECTION TEST SUMMARY - {title_suffix}")
    print("="*90)
...
def main():
    ...
    if ray_data_files:
        ...
        for file in ray_data_files[:10]:  # Show first 10 files
            print(f"  - {file}")
        if len(ray_data_files) > 10:
            print(f"  ... and {len(ray_data_files) - 10} more files")
    else:
    ...
    # Create chart with Median-16
    print("\nCreating stacked bar chart WITH Median-16...")
    result_df_with = create_stacked_bar_chart(aggregated_df, include_sah16=True)
    print_detailed_summary(aggregated_df, include_sah16=True)

    # Create chart without Median-16
    print("\nCreating stacked bar chart WITHOUT Median-16...")
    result_df_without = create_stacked_bar_chart(aggregated_df, include_sah16=False)
    print_detailed_summary(aggregated_df, include_sah16=False)
